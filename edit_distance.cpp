...
const double THRES_MULT =
    3; // number of standard deviations above the average to place threshold
...
size_t match_score(char *str1, size_t str1_len, char *str2, size_t str2_len) {
  ...
  for (size_t i = 0; i < str2_len + 1; i++) {
    ...
    for (size_t j = 1; j < str1_len + 1; j++) {
      ...
      size_t mm_gap =
          prev_line[j - 1] + (str2[i - 1] == str1[j - 1] ? ALPHA : BETA);
...
void *compare_pair(void *arg_ptr) {
  ...
  size_t score = match_score(args->str1, strlen(args->str1), args->str2,
                             strlen(args->str2));
...
int main(int argc, char *argv[]) {
  ...
  for (auto &row : reader) {
    try {
      ...
      bool cond1 = (question_order.find(question) != question_order.end()) &&
                   score == 100 && (double(duration) / attempts <= 25.0) &&
                   attempts < 10;

      bool cond2 = (question_order.find(question) != question_order.end()) &&
                   attempts <= 2 && score == 100 && duration < 120;
      ...
      cerr << endl
           << "[ERROR] Skipping row due to exception: " << e.what() << endl;
      return -1;
    }
  }
  cout << "Done" << endl;
  ...
  if (filtered_rows.empty()) {
    ...
    sort(filtered_rows.begin(), filtered_rows.end(),
    ...
    for (auto &r : filtered_rows) {
      ...
      odd_times << netid << ":" << (netid.length() > 6 ? "\t" : "\t\t")
                << "Attempts:" << r.Number_attempts
  ...
  curr_dur = chrono::duration_cast<chrono::milliseconds>(end - begin).count() *
             1.0 / 1000.0;
  cout << "[LOG] Runtime Duration: " << curr_dur << "[s]" << endl;
  total_duration += curr_dur;
  begin = end;

  // Section 2: Run an alignment program that finds students with high matching
  // scores
  ...
  sort(net_id_list.begin(), net_id_list.end(),
       [](string a, string b) { return a < b; });
  sort(student_dir_list.begin(), student_dir_list.end(),
       [](string a, string b) { return a < b; });
  ...
  for (size_t ques_idx = 0; ques_idx < questions.size(); ques_idx++) {
    ...
    for (auto student : student_dir_list) {
      for (const auto &entry : fs::directory_iterator(student)) {
        ...
        // Name of question goes across a '/' character but between 2nd and 3rd
        // '_' character of full directory
        ...
        if (questions[ques_idx] == q) {
          ...
          if (!file) {
            cerr << "[ERROR] Could Not Read Reponse File of Student: "
                 << curr_path.substr(curr_path.find('/', 2), first) << endl;
            return -1;
          }
    ...
    vector<vector<size_t>> curr_question_match_scores(
        len, vector<size_t>(len, 0)); // match score table

    if (len < 2) {
      cout << "[LOG] " << "Length of Question " << questions[ques_idx]
           << " Too Short:" << len << endl;
      ;
      continue;
    }
    ...
    for (size_t i = 0; i < len; i++) {
      for (size_t j = i + 1; j < len; j++) {
        ThreadContext *args = new ThreadContext{curr_question_responses[i],
    ...
    for (size_t i = 0; i < len; i++) {
      for (size_t j = i + 1; j < len; j++) {
        ...
        size_t diff = (temp > match_score_mean ? temp - match_score_mean
                                               : match_score_mean - temp);
    ...
    // Go through all scores and compute highest matching student pairs and
    // record how often a student's answer was flagged
    ...
    // highest matching student pairs is sorted based on how high the match
    // score is
    vector<pair<string, size_t>> net_id_freq_vec(net_id_freq_map.begin(),
                                                 net_id_freq_map.end());
    sort(net_id_freq_vec.begin(), net_id_freq_vec.end(),
    ...
    curr_dur =
        chrono::duration_cast<chrono::milliseconds>(end - begin).count() * 1.0 /
        1000.0;
    cout << "[LOG] Runtime Duration: " << curr_dur << "[s]" << endl;
    total_duration += curr_dur;
    begin = end;
